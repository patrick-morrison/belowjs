...
- AR now also retires sessions that remain hidden without inputs, and mirrors Three.js VR mode by keeping a single native `offerSession()` re-entry path available after a session ends.
